...
class RNNLM_Model(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.L = nn.Embedding(self.config.vocab_size, self.config.embed_size)
    self.I = nn.Parameter(torch.empty((self.config.embed_size, self.config.hidden_size), device = 'cuda'))
    self.H = nn.Parameter(torch.empty((self.config.hidden_size, self.config.hidden_size), device = 'cuda'))
    self.b1 = nn.Parameter(torch.empty((self.config.hidden_size), device = 'cuda'))
    self.U = nn.Parameter(torch.empty((self.config.hidden_size, self.config.vocab_size), device = 'cuda'))
    self.b2 = nn.Parameter(torch.empty((self.config.vocab_size), device = 'cuda'))

    #self.input_drop = self.config.dropout
    #self.output_drop = self.config.dropout
    ### END YOUR CODE
  ...
  def add_embedding(self, input_x):
    ...
    ### YOUR CODE HERE
    output = list()
    for j in range(input_x.size()[1]):
        output.append(self.L(input_x[:,j]))
  ...
  def add_model(self, input_x, initial_state):
    ...
    ### YOUR CODE HERE
    final_state = torch.zeros(self.config.batch_size, self.config.hidden_size)
    rnn_outputs = [(torch.zeros(self.config.batch_size, self.config.hidden_size)) for i in range(len(input_x))]
    initial_state = initial_state.cuda()
    for i in range(self.config.batch_size):
        temp_list = list()
        for j in range(len(input_x)):
            if(j == 0): hidden_state = torch.sigmoid(torch.matmul(initial_state[i],self.H) + torch.matmul(input_x[j][i],self.I) + self.b1)
            else: hidden_state = torch.sigmoid(torch.matmul(hidden_state,self.H) + torch.matmul(input_x[j][i],self.I) + self.b1)
            rnn_outputs[j][i] = hidden_state
        final_state[j] = hidden_state
  ...
  def add_projection(self, rnn_outputs):
    ...
    outputs = [(torch.zeros(self.config.batch_size, self.config.vocab_size)) for i in range(len(rnn_outputs))]
    for i in range(self.config.batch_size):
        temp_list = list()
        for j in range(len(rnn_outputs)):
            outputs[j][i] = m(torch.matmul(rnn_outputs[j][i].cuda(),self.U) + self.b2)
...
def compute_loss(ouputs, y, criterion):
  ...
  """
  ### YOUR CODE HERE
  temp_outputs = torch.empty((ouputs[0].size()[0], len(ouputs)*ouputs[0].size()[1]), device = 'cuda')

  for i in range(ouputs[0].size()[0]):
      temp_tensor = ouputs[0][i]
      for j in range(1, len(ouputs)):
          temp_tensor = torch.cat((temp_tensor, ouputs[j][i]), 0)
      temp_outputs[i] = temp_tensor

  temp_outputs = torch.reshape(temp_outputs, [640, 10000])
  y = torch.reshape(y, [640,]) # reshaping y
  loss = criterion(temp_outputs, y)
  ### END YOUR CODE
  return loss
...
def run_epoch(our_model, config, model_optimizer, criterion, data, mode='train', verbose=10):
  ...
  for step, (x, y) in enumerate(ptb_iterator(data, config.batch_size, config.num_steps)):
      ...
      if verbose and step % verbose == 0:
        sys.stdout.write('\r{} / {} : pp = {}'.format(step, total_steps, np.exp(np.mean(total_loss)), np.mean(total_loss)))
        sys.stdout.flush()
  if verbose:
      sys.stdout.write('\r')
