...
class Config(object):
  ...
  max_epochs = 5 # the number of max epoch
  early_stopping = 5
  dropout = 0.1
  lr = 0.01
  vocab_size= 0
...
class RNNLM_Model(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.I = nn.Linear(self.config.embed_size, self.config.hidden_size)
    self.H = nn.Linear(self.config.hidden_size, self.config.hidden_size)
    self.b1 = nn.Parameter(torch.empty((self.config.hidden_size), device = 'cuda'))
    self.U = nn.Linear(self.config.hidden_size, self.config.vocab_size)
    self.b2 = nn.Parameter(torch.empty((self.config.vocab_size), device = 'cuda'))

    self.input_drop = nn.Dropout(p=self.config.dropout)
    self.output_drop = nn.Dropout(p=self.config.dropout)
    ### END YOUR CODE
  ...
  def add_model(self, input_x, initial_state):
    ...
    for i in range(self.config.batch_size):
        ...
        for j in range(len(input_x)):
            input_1 = self.I(input_x[j][i])
            if(j == 0):
                input_2 = self.H(initial_state[i])
            else:
                input_2 = self.H(hidden_state)
            hidden_state = torch.sigmoid(input_2 + input_1 + self.b1)
  ...
  def add_projection(self, rnn_outputs):
    ...
    for i in range(self.config.batch_size):
        ...
        for j in range(len(rnn_outputs)):
            input_1 = self.U(rnn_outputs[j][i].cuda())
            outputs[j][i] = m(input_1 + self.b2)
...
def compute_loss(ouputs, y, criterion):
  ...
  """
  ### YOUR CODE HERE
  batch_size = ouputs[0].size()[0]
  num_step = len(ouputs)
  vocab_size = ouputs[0].size()[1]

  temp_outputs = torch.empty((batch_size, num_step*vocab_size), device = 'cuda')

  for i in range(ouputs[0].size()[0]):
      temp_tensor = ouputs[0][i]
      for j in range(1, len(ouputs)):
          temp_tensor = torch.cat((temp_tensor, ouputs[j][i]))
      temp_outputs[i] = temp_tensor

  temp_outputs = torch.reshape(temp_outputs, [batch_size*num_step, vocab_size])
  y = torch.reshape(y, [batch_size*num_step,]) # reshaping y
...
def test_RNNLM():
  ...
  model_optimizer = torch.optim.AdamW(our_model.parameters(), lr=config.lr)
  ### END YOUR CODE
